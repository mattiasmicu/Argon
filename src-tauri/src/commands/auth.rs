...
// Opens microsoft.com/link in an in-app webview window
#[command]
...
pub async fn poll_device_auth(
    ...
    loop {
        ...
        let token: MSTokenResponse = serde_json::from_str(&text)
            .map_err(|e| format!("Poll parse failed: {} — body: {}", e, text))?;
        ...
        match token.error.as_deref() {
            ...
            Some(other) => {
                return Err(format!("Auth error: {}", other));
            }
            None => {
                ...
                return run_xbox_chain(access_token, refresh_token).await;
            }
        }
    }
}
